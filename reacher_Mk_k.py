...
def main():
    ...
    parser.add_argument("--sigma_bar", type=float, default=0.55)
    ...
    sigma_min_Mk_history = deepc.get_sigma_min_Mk_history()
    solve_time_ms_history = deepc.get_solve_time_ms_history()
    input_history = deepc.get_input_history()
    solver_status_history = deepc.get_status_history()
    slack_norm_inf_history = deepc.get_slack_norm_inf_history()

    # 상위 5% threshold
    slack_thr = np.quantile(slack_norm_inf_history, 0.95)
    slack_spike = slack_norm_inf_history >= slack_thr

    t_ms = 1000*np.array(solve_time_ms_history)         # (T,)  fallback

    T = min(len(sigma_min_Mk_history), len(input_history), len(t_ms))
    sigma_min_Mk_history, input_history, t_ms = sigma_min_Mk_history[:T], input_history[:T], t_ms[:T]

    u_max=0.5
    u_min=-0.5
    sat_eps = 1e-3
    sat_hit = np.max(np.abs(input_history), axis=-1) >= (u_max - sat_eps)

    t_thr = np.quantile(t_ms, 0.95)
    time_spike = t_ms >= t_thr

    sigma_bar = 0.1             # threshold
    k = np.arange(T)
    import matplotlib.pyplot as plt

    plt.figure(dpi=160)
    plt.plot(k, sigma_min_Mk_history, linewidth=1.5, label=r'$\sigma_{\min}(M_k)$')
    plt.axhline(sigma_bar, linestyle='--', linewidth=1.2, label=r'$\sigma_{\mathrm{bar}}$')

    plt.scatter(k[sat_hit], sigma_min_Mk_history[sat_hit], s=14, label='torque saturation')
    plt.scatter(k[time_spike], sigma_min_Mk_history[time_spike], s=14, label='solve-time spike')
    plt.scatter(k[slack_spike], sigma_min_Mk_history[slack_spike],s=20, marker='x', label='slack violation')
    plt.xlabel('step k')
    plt.ylabel(r'conditioning margin  $\sigma_{\min}(M_k)$')
    plt.grid(True, which='both', alpha=0.3)
    plt.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(args.outdir, 'fig_sigma_events.png'))
    plt.show()
