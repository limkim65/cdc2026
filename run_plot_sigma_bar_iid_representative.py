...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Plot sigma_bar-sweep iid representative 9-metrics figure."
    )
    parser.add_argument(
        "--logdir",
        type=str,
        default=os.path.join("logs", "reacher", "sigma_bar_sweep", "dataset_iid"),
        help="Directory containing run_fixedk_K*_sigma*_seed*.npz files.",
    )
    parser.add_argument(
        "--outdir",
        type=str,
    ...
    parser.add_argument(
        "--fig_name",
        type=str,
        default="fig_sigma_bar_iid_representative_9metrics.png",
    )
    parser.add_argument(
        "--csv_name",
        type=str,
        default="sigma_bar_stats_representative.csv",
    )
    parser.add_argument("--dpi", type=int, default=220)
    return parser.parse_args()


def _extract_sigma_bar(path, data):
    if "sigma_bar" in data.files:
        return float(np.asarray(data["sigma_bar"]).item())
    m = re.search(r"_sigma([0-9.]+)_seed", os.path.basename(path))
    if m is None:
        raise ValueError(f"Could not parse sigma from filename: {path}")
    return float(m.group(1))


def load_runs(logdir):
    paths = sorted(glob.glob(os.path.join(logdir, "run_fixedk_K*_sigma*_seed*.npz")))
    if len(paths) == 0:
        paths = sorted(glob.glob(os.path.join(logdir, "run_sigma_bar_sweep_*_seed*.npz")))
    if len(paths) == 0:
        raise FileNotFoundError(
            "No supported npz files found in: "
            f"{logdir} (expected run_fixedk_K*_sigma*_seed*.npz or run_sigma_bar_sweep_*_seed*.npz)"
        )
    ...
    for path in paths:
        ...
        sigma_bar = _extract_sigma_bar(path, d)
    ...
    return pd.DataFrame(rows).sort_values(["sigma_bar", "seed"])


def aggregate_representative(df):
    rep_rows = []
    for sigma_bar, sub in df.groupby("sigma_bar"):
        row = {"sigma_bar": float(sigma_bar)}
        ...
        rep_rows.append(row)
    return pd.DataFrame(rep_rows).sort_values("sigma_bar")


def plot_representative(rep_df, out_path, dpi):
    x = rep_df["sigma_bar"].to_numpy(dtype=float)
    ...
    for i, (metric, label) in enumerate(METRICS):
        ...
        axs[i].set_title(f"{label} vs sigma_bar (representative)")
        axs[i].set_xlabel("sigma_bar")
