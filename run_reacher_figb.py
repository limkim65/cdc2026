...
from select_deepc.deepc_controller import SelectDeePC_analyze  # noqa: E402
...
def main():
    ...
    reference=scheduler._targets.targets[scheduler._target_idx]
    S_k_history=deepc._selected_idcs_history
    K_list = list(range(1, len(S_k_history), 10))
    for K in K_list:
        S_k=S_k_history[K]
        Hu=Hu_all[:, S_k]
        Hy=Hy_all[:, S_k]
        deepc._H_u=Hu
        deepc._H_y=Hy
        y_traj=deepc.y_past_history[K]
        u_traj=deepc.u_past_history[K]
        
        deepc.compute_action(y_traj[S_k], reference)
